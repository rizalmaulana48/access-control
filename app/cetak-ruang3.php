...
$html = '<!DOCTYPE html>
    ...
    http://vokasi.unpak.ac.id email: sekretariat.diploma3@unpak.ac.id</p>
            <hr>
            <table>
            <tr> <td><p style="float:left;font-size:120%;" >Kode Ruang</td><td> :</td><td><p style="float:left;font-size:120%;" >RK.3.3</td></tr>
            ...
            <table  border="1" cellpadding="10" cellspacing="0">
                ...
                $i=1;
                if( $from != 0 AND $end !=0){
                    $rekap = mysqli_query($koneksi, "SELECT * FROM akses_kontrol INNER JOIN ruang ON ruang.id_ruang = akses_kontrol.id_ruang WHERE ruang.id_ruang='3' AND tanggal BETWEEN '$from' AND '$end'");
                    foreach ($rekap as $dt){                            
                      $html.= '<tr> 
                    <td>'. $i++ .'</td>
                    <td>'. $dt["nama"] .'</td>
                    ...
                    $rekap = mysqli_query($koneksi, "SELECT * FROM akses_kontrol INNER JOIN ruang ON ruang.id_ruang = akses_kontrol.id_ruang WHERE ruang.id_ruang='3'");
                    foreach ($rekap as $dt){
                        $html.= '<tr> 
                    <td>'. $i++ .'</td>
                    <td>'. $dt["nama"] .'</td>
